...
public class SparseRow {
  ...
  * @throws IOException if the JSON Element is invalid with respect to SparseRow
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SparseRow.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SparseRow.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SparseRow` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("cells") != null && !jsonObj.get("cells").isJsonNull()) {
        ...
        if (jsonArraycells != null) {
          ...
          for (int i = 0; i < jsonArraycells.size(); i++) {
            ChangeDataCell.validateJsonElement(jsonArraycells.get(i));
          };
        }
      }
  }
