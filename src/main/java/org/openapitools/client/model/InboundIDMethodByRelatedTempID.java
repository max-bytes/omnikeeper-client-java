...
public class InboundIDMethodByRelatedTempID extends AbstractInboundIDMethod {
  ...
  * @throws IOException if the JSON Element is invalid with respect to InboundIDMethodByRelatedTempID
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InboundIDMethodByRelatedTempID.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InboundIDMethodByRelatedTempID.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InboundIDMethodByRelatedTempID` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
