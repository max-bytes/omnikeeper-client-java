...
public class GenericInboundData {
  ...
  private List<GenericInboundCI> cis = new ArrayList<>();

  public static final String SERIALIZED_NAME_RELATIONS = "relations";
  @SerializedName(SERIALIZED_NAME_RELATIONS)
  private List<GenericInboundRelation> relations = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to GenericInboundData
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GenericInboundData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GenericInboundData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GenericInboundData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("cis") != null && !jsonObj.get("cis").isJsonNull()) {
        ...
        if (jsonArraycis != null) {
          ...
          for (int i = 0; i < jsonArraycis.size(); i++) {
            GenericInboundCI.validateJsonElement(jsonArraycis.get(i));
          };
        }
      }
      if (jsonObj.get("relations") != null && !jsonObj.get("relations").isJsonNull()) {
        ...
        if (jsonArrayrelations != null) {
          ...
          for (int i = 0; i < jsonArrayrelations.size(); i++) {
            GenericInboundRelation.validateJsonElement(jsonArrayrelations.get(i));
          };
        }
      }
  }
